...
class EmpID{
 ...
 public:
   ...
   bool operator==(const EmpID &d1){
	if(d1.empid==this->empid){
		return true;
	}
  ...
  friend void operator>>(std::istream &in,EmpID &d1){
    int k;
    in>>k;
    d1.empid=k;
  }
  
    friend void operator<<(std::ostream &out,EmpID &d1){
...
E1.insertelems(num);
E1.display_dups(E1.findduplicate());
